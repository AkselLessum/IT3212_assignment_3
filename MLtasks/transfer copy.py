...
svm_sep = SVC(kernel='linear', C=1, probability=True)
svm_sep.fit(X_target_train_pca, y_target_train)
pred_sep = svm_sep.predict(X_target_test_pca) # Predict on the test set, only small model
print("\"Separate\" SVM accuracy score (Linear): ", accuracy_score(y_target_test, pred_sep))
#"BIG MODEL" SVM accuracy score (Linear):  0.8203517587939698
#"Transfer" SVM accuracy score (Linear):  0.7303370786516854

# Accuracy scores
big_model_accuracy = accuracy_score(y_source_test, pred_linear)
transfer_model_accuracy = accuracy_score(y_target_test, pred_final)

# Plotting
labels = ['90%', 'Transfered']
accuracies = [big_model_accuracy, transfer_model_accuracy]
